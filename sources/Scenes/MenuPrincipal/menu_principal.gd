...
func _reload_players() -> void:
	...
	for nom in names:
		...
		_add_player_theme(button, str(nom))
...
func _add_player_theme(button: Button, player_name: String) -> void:
	var selected := SauvegardeBddJoueursService.le_joueur_existe() and SauvegardeBddJoueursService.lire_nom_joueur() == player_name
